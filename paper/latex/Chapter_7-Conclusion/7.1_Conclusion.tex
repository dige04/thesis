...
\begin{todobox}
Expand the beats below into prose. RESTATE the study, recap the metric \textrightarrow{} trade-off, end by naming the contribution as a template, not a new policy. Thread "retrieval held constant", "non-inferiority", "footprint", "not every task needs memory". Numbers stay placeholders, pending the 144-run rerun.
\end{todobox}
...
% - METRIC -> TRADE-OFF RECAP: on the CL-F1 proxy, forgetting is non-inferior to full-memory
%   accumulation [RESULT: pending] at materially lower memory footprint [RESULT: pending] --
%   the footprint axis (A4) is where forgetting wins. (Non-inferiority via bounded CIs +
%   power, not TOST.)
% - THE REFRAME: because only a minority of tasks are interdependent (benchmark-structural;
%   exact fractions pending), memory is best understood as a DOSAGE / TARGETING question --
%   "not every task needs memory" -- rather than an always-on accumulation default.
