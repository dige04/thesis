...
class TestRecencyPrunePolicyMaintain:
    ...
    **Validates: Requirements 11.3, 11.4**
    ...
    def test_maintain_archives_correct_number(self):
        """Verify maintain() archives exactly enough to reach max_records (Req 11.4)."""
...
class TestRecencyPrunePolicyOldestFirst:
    """Test CRITICAL invariant: Archives oldest by sequence_index.

    **Validates: Requirements 11.3**

    This is a frozen invariant from the task description and design document.
    Archives oldest memories by sequence_index (chronological order).
    """

    def test_archives_oldest_by_sequence_index(self):
    ...
    def test_archives_oldest_regardless_of_memory_type(self):
        """Verify pruning is based on sequence_index, not memory_type."""
        policy = RecencyPrunePolicy(max_records=5)
        store = MockMemoryStore()
        ...
        # Should archive oldest by sequence_index (0-4), regardless of type
        archived_seq_indices = sorted([r.sequence_index for r in store.archived_records])
        assert archived_seq_indices == [0, 1, 2, 3, 4]

    def test_archives_oldest_with_non_sequential_indices(self):
        """Verify pruning works correctly with non-sequential sequence_index."""
        policy = RecencyPrunePolicy(max_records=3)
        store = MockMemoryStore()

        # Add records with non-sequential sequence_index
        indices = [5, 2, 8, 1, 9, 3, 7]
        for idx in indices:
            store.records.append(create_mock_record(f"mem-{idx:03d}", idx))

        policy.maintain(store)

        # Should archive the 4 oldest (sequence_index 1, 2, 3, 5)
        archived_seq_indices = sorted([r.sequence_index for r in store.archived_records])
        assert archived_seq_indices == [1, 2, 3, 5]

        # Should retain the 3 most recent (sequence_index 7, 8, 9)
        active_seq_indices = sorted([r.sequence_index for r in store.active_records()])
        assert active_seq_indices == [7, 8, 9]


class TestRecencyPrunePolicyDeterminism:
    """Test that Recency Prune is deterministic (no randomness)."""
    ...
    def test_multiple_maintain_calls_are_consistent(self):
        """Verify multiple maintain() calls are consistent."""
        policy = RecencyPrunePolicy(max_records=5)
        store = MockMemoryStore()
        ...
        # Should have sequence_index 5-9 active
        active_seq_indices = sorted([r.sequence_index for r in store.active_records()])
        assert active_seq_indices == [5, 6, 7, 8, 9]
        ...
        # Should now have sequence_index 10-14 active (5 most recent)
        active_seq_indices = sorted([r.sequence_index for r in store.active_records()])
        assert active_seq_indices == [10, 11, 12, 13, 14]
...
class TestRecencyPrunePolicyEdgeCases:
    ...
    def test_maintain_with_max_records_one(self):
        ...
        # Should retain only the most recent (sequence_index 2)
